  ...
  <div class="card text-white">
    <div class="card-body">
      ...
      <div v-show="currentView === 'pie'">
        <canvas id="myChartTeachers" width="400" height="400"></canvas>
      </div>
...
<script setup>
  ...
  onMounted(async () => {
    try {
      const response = await axios.get('http://backend.vue-laravel-app.local/api/teachers');
      ...
      teachersWithoutCoursesCount.value = teachers.filter(teacher => !teacher.course_id).length;
        ...
        const myChartTeachers = new Chart(
          document.getElementById('myChartTeachers'),
          config
        );
